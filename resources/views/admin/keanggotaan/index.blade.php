...
@section('content')
    <div class="container mx-auto px-4 py-10 max-w-screen-lg">
        <h1 class="text-3xl font-bold mb-6">Data Keanggotaan</h1>
        <!-- Tombol Tambah Data -->
        @if (session('success'))
        ...
        <div class="mb-4">
            <a href="{{ route('keanggotaan.create') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($keanggotaans as $keanggotaan)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $keanggotaan->nama }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $keanggotaan->nim }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $keanggotaan->profesi }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $keanggotaan->asal_institusi }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="{{ route('keanggotaan.show', $keanggotaan->id) }}"
