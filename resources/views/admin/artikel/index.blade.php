...
@section('content')
    <div class="container mx-auto px-4 py-10 max-w-screen-lg">
        <h1 class="text-3xl font-bold mb-6">Data Artikel dan Berita</h1>
        <!-- Tombol Tambah Data -->
        @if (session('success'))
                        ...
                        @foreach ($data as $item)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <img src="{{asset($item->image)}}" width="50" height="50" alt=""></td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $item->title }}</td>
                               
                                <td class="px-6 py-4 whitespace-nowrap">{{ $item->text }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
