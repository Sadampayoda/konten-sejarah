...
@section('content')
    <div class="container py-10 flex justify-center">
        <div class="max-w-lg w-full bg-white shadow-md rounded-lg overflow-hidden">
            ...
            @endif
            <div class="p-6">
                <h4 class="text-2xl font-semibold text-blue-gray-900 mb-1">Edit Kontak</h4>
                <p class="text-base text-gray-700 mb-4">Edit Kontak Anda di sini</p>
                <form method="POST" action="{{ route('kontak.update', $data->id) }}" class="w-full">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">name</label>
                        <input name="name" value="{{$data->name}}" id="name" type="text"
                    ...
                    <div class="mb-4">
                        ...
                        <input name="subject" value="{{$data->subject}}" id="subject" type="text"
                            class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div class="mb-4">
                        <label for="message" class="block text-sm font-medium text-gray-700 mb-1">message</label>
                        <input name="message" value="{{$data->message}}" id="message" type="number"
                            class="block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <button type="submit"
