...
@section('content')
    <div class="container mx-auto px-4 py-10 max-w-screen-lg">
        <h1 class="text-3xl font-bold mb-6">Data Medsos</h1>
        <!-- Tombol Tambah Data -->
        @if (session('success'))
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <form action="{{ route('medsos.edit',$item->id) }}" method="GET"
                                        class="inline">
                                        
                                        <button type="submit" name="id" value="{{$item->id}}" class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded">Edit</button>
                                    </form>
                                    <form action="{{ route('medsos.destroy', $item->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" name="id" value="{{$item->id}}" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
